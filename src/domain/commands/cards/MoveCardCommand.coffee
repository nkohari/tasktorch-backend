  ...
  execute: (conn, callback) ->
    ...
    conn.execute statement, (err, previousStacks) =>
      ...
      conn.execute statement, (err, currentStack) =>
        ...
        # NB: Cards should only be in one stack at a time. The RemoveCardFromStackStatement
        ...
        if previousStack.id != currentStack.id
          patch.moves = r.row('moves').append(new Move(@user, previousStack, currentStack))
